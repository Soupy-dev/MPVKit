...
root = Path('/Users/nihad/Documents/MPVKit')
work = root / '.build/macos-native-local'
args = argparse.Namespace(platform='macos', arch='arm64')
variant = None
sdk_name = 'macosx'
...
meson_source = source_copy / 'meson.build'
meson_text = meson_source.read_text()
coreaudio_sources = "    dependencies += coreaudio\n    sources += files('audio/out/ao_coreaudio.c',"
coreaudio_helpers = "    dependencies += coreaudio\n    if not features['cocoa']\n        sources += files('osdep/utils-mac.c')\n    endif\n    sources += files('audio/out/ao_coreaudio.c',"
if coreaudio_sources in meson_text:
    meson_source.write_text(meson_text.replace(coreaudio_sources, coreaudio_helpers, 1))
elif coreaudio_helpers not in meson_text:
    raise RuntimeError('Unexpected CoreAudio helper source configuration')
artifacts = {}
...
for name, artifact in artifacts.items():
    ...
    if len(matches) != 1:
        raise RuntimeError('Missing exact macOS architecture for ' + name)
    item = matches[0]
...
lines = ["[binaries]", "c = '/usr/bin/clang'", "cpp = '/usr/bin/clang++'", "objc = '/usr/bin/clang'", "objcpp = '/usr/bin/clang++'", "ar = '/usr/bin/ar'", "strip = '/usr/bin/strip'", "pkg-config = '/opt/homebrew/bin/pkg-config'", '', '[properties]', 'has_function_printf = true', 'has_function_hfkerhisadf = false', '', '[host_machine]', "system = 'darwin'", "subsystem = '" + 'macos' + "'", "kernel = 'xnu'", "cpu_family = '" + family + "'", "cpu = '" + args.arch + "'", "endian = 'little'", '', '[built-in options]', "default_library = 'static'", "buildtype = 'release'", "prefix = '" + str(work / f'installed-{args.platform}-{args.arch}') + "'"]
...
options = ['-Dlibmpv=true', '-Dgl=enabled', '-Dplain-gl=enabled', '-Diconv=enabled', '-Duchardet=enabled', '-Dvulkan=enabled', '-Dmoltenvk=enabled', '-Dios-sample-buffer=disabled', '-Dapple-gpu-pip=enabled', '-Djavascript=disabled', '-Dzimg=disabled', '-Djpeg=disabled', '-Dvapoursynth=disabled', '-Drubberband=disabled', '-Dgpl=true', '-Dlibbluray=enabled', '-Dcplayer=false', '-Dvideotoolbox-gl=disabled', '-Dvideotoolbox-pl=enabled', '-Dswift-build=disabled', '-Daudiounit=disabled', '-Davfoundation=disabled', '-Dcoreaudio=enabled', '-Dlua=disabled', '-Dios-gl=disabled', '-Dcocoa=disabled', '-Dgl-cocoa=disabled']
